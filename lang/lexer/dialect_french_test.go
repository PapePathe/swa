...
func TestPatternsFrench(t *testing.T) {
	...
	tests := []struct {
		...
		{patterns[3].regex, `si`, defaultHandler(KeywordIf, "if")},
		{patterns[4].regex, `sinon`, defaultHandler(KeywordElse, "else")},
		{patterns[5].regex, `structure`, defaultHandler(Struct, "struct")},
